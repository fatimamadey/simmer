    ...
    <article className="paper-panel overflow-hidden rounded-[26px] transition duration-300 hover:-translate-y-1 hover:shadow-[0_26px_56px_rgba(83,19,30,0.14)]">
      ...
      <div className="space-y-4 p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <Link href={`/profile/${post.author.username}`} className="text-xs uppercase tracking-[0.18em] text-[color:rgba(90,70,76,0.72)] hover:text-[var(--oxblood)]">
        ...
        {post.notes?.trim() ? (
          <p className="line-clamp-3 text-[1.02rem] leading-7 text-[color:rgba(61,45,51,0.78)]">{post.notes.trim()}</p>
        ) : null}
